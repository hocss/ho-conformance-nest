...
export default class Nest extends Base {
    ...
    onRuleset( rule ) {
        ...
        if ( children.length > 0 ) {
            ...
            this.log( chalk.cyan( this.tree.pwd, this._parseSelectors( rule.raw.selectors ), this.tree.current.numChildren ) )
            return
        }

        this.tree.current.numChildren--

        if ( this.tree.current.numChildren < 0 ) {
